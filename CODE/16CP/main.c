...
#define MV16_ON (P1 &= ~0x40)
#define MV16_OFF (P1 |= 0x40)
...
void CAN0_Init (void)
{
   ...
   // Arbitration Registers
   CAN0IF1A2 = 0x8000 | (CP16_ID << 2);  // Set MsgVal to valid
                                         // Set Object Direction to read
                                         // Set 11-bit Identifier to iter

   CAN0IF1CR = CP16_ID;                // Start command request
   ...
   // Arbitration Registers
    CAN0IF1A2 = 0xA000 | (CP13_ID << 2);  // Set MsgVal to valid
   ...
   CAN0IF1A2 = 0xA000 | (CP20_ID << 2);  // Set MsgVal to valid
                                         // Set Direction to write
                                         // Set 11-bit Identifier to iter

   CAN0IF1CR = CP20_ID;                // Start command request
   ...
   {
   	if(iter != CP20_ID && iter != CP16_ID && iter != Broadcast_ID && iter != IPM_ID && iter != CP13_ID)
	 {
...
INTERRUPT (CAN0_ISR, INTERRUPT_CAN0)
{
   ...
   if (status & RxOk )                  // If transmit completed successfully
   {
      ...
      CAN_RX_COMPLETE = 1;
	if(Interrupt_ID == CP16_ID && CAN_Rx_Buf[3] == BPCP_ID && CAN_Rx_Buf[4] == 0x00)
	{
		Rx_BPCP_OK = 1;
	}
