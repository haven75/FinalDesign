...
#define MV53_ON (P1 &= ~0x40)
#define MV53_OFF (P1 |= 0x40)
#define MVEM_ON ( P2 &= ~0x40)
#define MVEM_OFF (P2 |= 0x40)
#define EMV_ON (P2 &= ~0x80)
#define EMV_OFF (P2 |= 0x80)
...
U8 FaultCode = 0x00;               //report fault
...
void CAN0_Init (void)
{
   ...
   // Arbitration Registers
   CAN0IF1A2 = 0x8000 | (BPCP_ID << 2);  // Set MsgVal to valid
   ...
   // Arbitration Registers
   CAN0IF1A2 = 0xA000 | (CP16_ID << 2);  // Set MsgVal to valid
   ...
   {
   	if(iter != BPCP_ID && iter != CP16_ID && iter != Broadcast_ID && iter != IPM_ID)
	 {
...
INTERRUPT (CAN0_ISR, INTERRUPT_CAN0)
{
   ...
   if (status & RxOk )                  // If transmit completed successfully
   {
	...
	if(Interrupt_ID == BPCP_ID && CAN_Rx_Buf[3] == ERCP_ID && CAN_Rx_Buf[4] == 0x00)
	{
		Rx_ERCP_OK = 1;
	}
