...
#define ERBU_ON (P1 &= ~0x40)
#define ERBU_OFF (P1 |= 0x40)
#define MV13S_ON ( P2 &= ~0x40)
#define MV13S_OFF (P2 |= 0x40)
#define MRCUT_ON (P2 &= ~0x80)
#define MRCUT_OFF (P2 |= 0x80)
...
void CAN0_Init (void)
{
   ...
   // Arbitration Registers
   CAN0IF1A2 = 0x8000 | (CP13_ID << 2);  // Set MsgVal to valid
                                         // Set Object Direction to read
                                         // Set 11-bit Identifier to iter

   CAN0IF1CR = CP13_ID;                // Start command request
   ...
   {
   	if(iter != IPM_ID && iter != Broadcast_ID && iter != CP13_ID)
	 {
...
INTERRUPT (CAN0_ISR, INTERRUPT_CAN0)
{
   ...
   if (status & RxOk )                  // If transmit completed successfully
   {
	...
	if(Interrupt_ID == CP13_ID && CAN_Rx_Buf[3] == CP16_ID && CAN_Rx_Buf[4] == 0x00)
		Rx_CP16_OK = 1;
	if(Interrupt_ID == Broadcast_ID && CAN_Rx_Buf[3] == IPM_ID)         //Check Information
	{	
		if( CAN_Rx_Buf[1] == 0x01 )
		{
			switch(CAN_Rx_Buf[0])
			{
				case 0x01: Yunzhuanwei(); break;       //Go to Yunzhuanwei
